    ...
    int countErr_chPr = 0;
      ...
      *dst = (s21_decimal){{1 << 23 | fltUnion1.mntsa}};
      ...
      while (expFlt != 0) {
        ...
        // 95 < expFlt
        // if (shift == -1 && dst->bits[3] & 1U << 31) {
        //   countErr_chPr += S21_changePrecision(dst, -1);
        //   err = 1;  //  return 1, но *result = tResult;
        // }
        *dst = S21_bitsMove(*dst, shift);
        expFlt += shift;
      if (fltUnion1.sign) s21_(negate)(*dst, dst);
    }
  S21_rounding(*dst, dst, TRUNCATE, dst->exp - countErr_chPr);
  S21_rounding(*dst, dst, TRUNCATE, __EXPLMT);
  S21_zeroRemover(dst);
  }
  return err;
}
