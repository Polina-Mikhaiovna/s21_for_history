...
int get_bit(const s21_decimal decVar, int bit) {
  int res = 0;
  if (bit / 32 < 4) {
    unsigned int mask = 1u << (bit % 32);  // литерал 'u' чтобы 1 интерепретировалась как unsigned
...
s21_decimal s21_add(s21_decimal number_1, s21_decimal number_2) {
  ...
  if (res.value_type == s21_NORMAL_VALUE || res.value_type == s21_ADDCODE) {
    if (!get_sign(&number_1) && !get_sign(&number_2)) {
      ...
      // создает доп код и снова вызывает сложение
      set_sign(&number_1, 0);
...
s21_decimal s21_sub(s21_decimal number_1, s21_decimal number_2) {
  ...
  if (res.value_type == s21_NORMAL_VALUE) {
    ...
    if (get_sign(&number_1) != get_sign(&number_2)) {
      ...
      if (s21_is_equal(number_1, number_2) == FALSE) {
        ...
        if (s21_is_less(number_1, number_2) == FALSE) {
          smallPtr = &number_1;
          bigPtr = &number_2;
          resultSign = !sign2;
        } else {
...
s21_decimal check_for_mul(s21_decimal number_1, s21_decimal number_2) {
  ...
  // проверки аргументов
  s21_decimal zero = {{0, 0, 0, 0}, s21_NORMAL_VALUE};
  int there_is_nan = is_NAN(&number_1, &number_2) == FALSE ? 1 : 0;
  int there_is_neg_inf = is_neg_inf(&number_1, &number_2) != 0 ? 1 : 0;
  int there_is_inf = is_inf(&number_1, &number_2) != 0 ? 1 : 0;
  int there_is_zero = s21_is_equal(number_1, zero) == FALSE ||
                              s21_is_equal(number_2, zero) == FALSE
                          ? 1
                          : 0;

  int there_is_plus_normal = (s21_is_greater(number_1, zero) == FALSE &&
                              number_1.value_type == s21_NORMAL_VALUE) ||
                                     (s21_is_greater(number_2, zero) == FALSE &&
                                      number_2.value_type == s21_NORMAL_VALUE)
                                 ? 1
                                 : 0;

  int there_is_neg_normal = (s21_is_less(number_1, zero) == FALSE &&
                             number_1.value_type == s21_NORMAL_VALUE) ||
                                    (s21_is_less(number_2, zero) == FALSE &&
                                     number_2.value_type == s21_NORMAL_VALUE)
                                ? 1
                                : 0;
 ...
 * @return TRUE - числа не NAN, FALSE - одно из децималов NAN
 */
int is_NAN(s21_decimal *dec1, s21_decimal *dec2) {
  int vt_dec1 = dec1->value_type;
  int vt_dec2 = dec2->value_type;

  return (vt_dec1 == s21_NAN || vt_dec2 == s21_NAN) ? FALSE : TRUE;
}
...
int zero_check(s21_decimal dec1, s21_decimal dec2) {
  return !!(dec1.bits[0] | dec2.bits[0] | dec1.bits[1] | dec2.bits[1] | dec1.bits[2] | dec2.bits[2]);
}
...
int s21_is_greater(s21_decimal dec1, s21_decimal dec2) {
  int is_greater = -1;
  if (is_NAN(&dec1, &dec2) == FALSE) is_greater = TRUE;
  ...
  if (is_greater == -1) {
    ...
    if (who_is_inf == 1) is_greater = FALSE;
    if (who_is_inf == -1 || who_is_inf == 2) is_greater = TRUE;

    int who_is_neg_inf = is_neg_inf(&dec1, &dec2);
    if (who_is_neg_inf == -1) is_greater = FALSE;
    if (who_is_neg_inf == 1 || who_is_inf == 2) is_greater = TRUE;
  }
  ...
  for (int i = 95; i >= 0 && is_greater == -1; i--) {
    ...
    if (bit_dec1 && !bit_dec2) is_greater = FALSE;
    if (bit_dec2 && !bit_dec1) is_greater = TRUE;
    if (is_greater != -1) {
...
int s21_is_equal(s21_decimal dec1, s21_decimal dec2) {
  int is_equal = -1;
  if (is_NAN(&dec1, &dec2) == FALSE) is_equal = TRUE;

  if (is_equal == -1) {
    //  if (zero_check(dec1, dec2) == FALSE) is_equal = FALSE;
    int who_is_inf = is_inf(&dec1, &dec2);
    if (who_is_inf == 1 || who_is_inf == -1) is_equal = TRUE;
    if (who_is_inf == 2) is_equal = FALSE;

    int who_is_neg_inf = is_neg_inf(&dec1, &dec2);
    if (who_is_neg_inf == 1 || who_is_neg_inf == -1) is_equal = TRUE;
    if (who_is_neg_inf == 2) is_equal = FALSE;
  }

  if (is_equal == -1) {
    int who_is_negative = is_negative(&dec1, &dec2);
    if (who_is_negative != 0) is_equal = TRUE;
  ...
  for (int i = 95; i >= 0 && is_equal == -1; i--) {
    int bit_dec1 = get_bit(dec1, i);
    int bit_dec2 = get_bit(dec2, i);
    if (bit_dec1 != bit_dec2) is_equal = TRUE;
  }

  return (is_equal != TRUE) ? FALSE : TRUE;
}
...
s21_decimal s21_div(s21_decimal divident, s21_decimal divisor) {
  ...
  s21_decimal zero = {{0, 0, 0, 0}, s21_NORMAL_VALUE};
  int divsr_is_normal_plus = s21_is_greater(divisor, zero) == FALSE &&
                                     divisor.value_type == s21_NORMAL_VALUE
                                 ? 1
                                 : 0;

  int divsr_is_normal_minus = s21_is_less(divisor, zero) == FALSE &&
                                      divisor.value_type == s21_NORMAL_VALUE
                                  ? 1
                                  : 0;
  ...
  if (is_NAN(&divident, &divisor) == FALSE) {
    result.value_type = s21_NAN;

  } else if (s21_is_equal(divisor, zero) == FALSE &&
             s21_is_less(divident, zero) == FALSE) {
    //    -x/0
    result.value_type = s21_NEGATIVE_INFINITY;

  } else if (s21_is_equal(divisor, zero) == FALSE &&
             s21_is_greater(divident, zero) == FALSE) {
    //   +x/0
    result.value_type = s21_INFINITY;

  } else if (s21_is_equal(divisor, zero) == FALSE &&
             s21_is_equal(divident, zero) == FALSE) {
    //    0/0
    result.value_type = s21_NAN;
  ...
  if (result.value_type == s21_NORMAL_VALUE && flag) {
    ...
    for (; inside_scale <= 27 && s21_is_equal(remainder, zero) == TRUE;) {
      if (s21_is_less(result, border_value) == TRUE) {
        break;
      }
 ...
 * @return Результат работы, где 0 - это TRUE, а 1 - это FALSE
 */
...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  dst->bits[0] = dst->bits[1] = dst->bits[2] = dst->bits[3] = 0;
  dst->value_type = s21_NORMAL_VALUE;
  int result = TRUE, sign = getFloatSign(&src), exp = getFloatExp(&src);
...
s21_decimal div_only_bits(s21_decimal number_1, s21_decimal number_2,
  ...
  for (int i = last_bit(number_1); i >= 0; i--) {
    ...
    if (s21_is_greater_or_equal(*buf, number_2) == FALSE) {
...
s21_decimal s21_round(s21_decimal dec1) {
  ...
  if (valid_value) {
    res = trunc;
    if (s21_is_greater_or_equal(buf, five) == FALSE) {
      res = s21_add(res, one);
    }
    set_sign(&res, sign);
  } else {
...
s21_decimal s21_floor(s21_decimal dec1) {
  ...
  if (s21_is_equal(dec1, dec1_copy) == FALSE) valid_value = 0;
